...
var Stage = (function (_super) {
    ...
    Stage.prototype._pointermove = function (evt) {
        ...
        if (Global_1.Konva.listenClickTap != null
            && Global_1.Konva.listenClickTap.pointerId === pointerId
            && Number.isFinite(evt.movementX)
            && Number.isFinite(evt.movementY)) {
            Global_1.Konva.listenClickTap.x += Math.abs(evt.movementX);
            Global_1.Konva.listenClickTap.y += Math.abs(evt.movementY);
        }
        var eventsEnabled = !DragAndDrop_1.DD.isDragging || Global_1.Konva.hitOnDragEnabled;
        if (eventsEnabled) {
    ...
    Stage.prototype._pointerdown = function (evt) {
        ...
        if (evt.isPrimary && (evt.pointerType !== 'mouse' || evt.button === 0)) {
            Global_1.Konva.listenClickTap = { pointerId: pointerId, x: 0, y: 0, at: Date.now() };
        }
        else {
            Global_1.Konva.listenClickTap = null;
        }
        clearTimeout(this.pressTimeout);
        if (evt.isPrimary && evt.pointerType !== 'mouse') {
            this.pressTimeout = setTimeout(function () {
                var firePress = Global_1.Konva.listenClickTap != null &&
                    Math.hypot(Global_1.Konva.listenClickTap.x, Global_1.Konva.listenClickTap.y) <= Global_1.Konva.clickDistance &&
    ...
    Stage.prototype._pointerup = function (evt) {
        ...
        var fireClick = Global_1.Konva.listenClickTap != null &&
            Global_1.Konva.listenClickTap.pointerId === pointerId &&
            Math.hypot(Global_1.Konva.listenClickTap.x, Global_1.Konva.listenClickTap.y) <= Global_1.Konva.clickDistance &&
            ((evt.pointerType === 'mouse' && evt.button === 0) ||
                (evt.pointerType !== 'mouse' && Date.now() < Global_1.Konva.listenClickTap.at + Global_1.Konva.clickWindow));
        Global_1.Konva.listenClickTap = null;
        clearTimeout(this.pressTimeout);
        if (fireClick) {
